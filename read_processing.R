...
processByChr <- function(tsDB, bam.chr, readLength, mc){
  
  # Process the paried read alignemtn
  frags <- getFragPairFromBam(bam.chr, readLength)
  
  z <- function(idx){
  	res <- processByGene(tsDB, idx, frags, readLength)
  }
...
#   cat(paste("\n-> using time", round(t_proc[3]/60,2) , " min. \n"))
  ...
  t_processing <- proc.time()
  res.chr <- mclapply(1:length(tsDB@transcripts), z, mc.cores=mc)
  t_proc <- proc.time() - t_processing
  cat(paste("\n-> using time", round(t_proc[3]/60,2) , " min. \n"))
...
processByGene <- function(tsDB, idx, frags, readLength=75){
